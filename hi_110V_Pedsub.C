  ...
  for(int i=0; i<Nchannels; i++)
    {
      ...
      PedCN_sub[i] = (TH1F*)f->GetDirectory("PedCN_sub")->Get(Form("PedCN_sub%i", i+1));
  ...
  for(int i=0; i<Nchannels; i++)
    {
      ...
      pedestal_sub[i]->GetXaxis()->SetRangeUser(-50, 50);
      ...
      c->SaveAs(Form("hi_110V_Pedsub_superimpose_plot/Ch%i.png", i+1));
    }
  

  //Write and Draw
  TFile *fit = new TFile("hi_110V_Pedsub.root", "RECREATE");
  for(int i=0; i<Nchannels; i++)
    {
      PedCN_sub[i]->Write();
    }
